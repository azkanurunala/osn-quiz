...
$args = @('scripts/record-videos.mjs','--concurrency',"$Concurrency",'--force','--music-every',"$MusicEvery")
if ($Only) { $args += @('--only', $Only) }
if ($Limit -gt 0) { $args += @('--limit', "$Limit") }

Write-Host "`nRunning record-videos $($args -join ' ')"
$env:NODE_NO_WARNINGS = '1'
node $args 2>&1 | Tee-Object -FilePath 'recordings\_batch-rerecord.log'
