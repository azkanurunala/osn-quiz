...
const APP_ROOT = 'c:/Products/osn-sd/osn-app';
...
const pendingIds = JSON.parse(readFileSync('C:/tmp/pending51.json', 'utf8'));
const items = pendingIds.map((id) => manifest.items.find((it) => it.id === id));

// Real video already exists for this one — pair it with the actual recording filename.
const VIDEO_OVERRIDE = {
  'ipa-01p-fotosintesis-metabolisme-campur': '016-OSN SD · IPA · Sub-bab IPA-01p · Fotosintesis & Metabolisme Tumbuhan',
};
...
// Disambiguate titles that repeat across tiers (same title text regardless of tier).
const titleCounts = {};
for (const it of items) {
  const s = sanitize(it.title);
  titleCounts[s] = (titleCounts[s] || 0) + 1;
}
...
async function main() {
  ...
  for (const item of items) {
    let base;
    if (VIDEO_OVERRIDE[item.id]) {
      base = `X-${VIDEO_OVERRIDE[item.id]}`;
    } else {
      let titlePart = sanitize(item.title);
      if (titleCounts[titlePart] > 1) {
        titlePart += ` (Tingkat ${TIER_LABEL[item.tier] || item.tier})`;
      }
      base = `X-${num}-${titlePart}`;
      num++;
    }
    ...
    ok++;
    console.log(`${ok}/${items.length}:`, base);
  }
  await browser.close();
  console.log(`\nDone. ${ok} pairs generated.`);
}
main();
