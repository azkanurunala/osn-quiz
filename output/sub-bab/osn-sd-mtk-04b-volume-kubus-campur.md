# OSN SD · MTK · Sub-bab MTK-04b · Volume Kubus

> Paket pembinaan sub-bab — 100 Soal PG dengan Pembahasan Analisis 4 Opsi
> **Kode sub-bab:** MTK-04b (Bab MTK-04 · Geometri Bangun Ruang & Volume)
> **Tingkat:** Campur — 50 Kabupaten + 30 Provinsi + 20 Nasional
> **Format:** 100% Pilihan Ganda (4 opsi A–D)
> **Kelas target:** 5–6 SD
> **Estimasi waktu pengerjaan:** 6–9 jam (boleh dibagi beberapa sesi)
> **Fokus drill:** 100 soal **hanya** pada sub-topik "Volume Kubus" — menghitung volume dari sisi (V = s³), mencari sisi dari volume (s = ∛V), konversi satuan volume (m³, dm³, cm³, mm³, liter, mL), dan aplikasi (bak air, kotak kado, dadu, kubus susun, debit pengisian).

---

## 📋 Daftar Isi

- **Bagian I — Ringkasan Rumus & Konversi Satuan Volume**
- **Bagian II — 100 Soal PG Lengkap** (Soal + 4 Pilihan + Kunci + Pembahasan analisis A/B/C/D)
- **Bagian III — Tabel Kunci & Refleksi**

---

# Bagian I — Ringkasan Rumus & Konversi Satuan Volume

## A. Rumus Inti Kubus

| Besaran | Rumus | Catatan |
|---|---|---|
| Volume kubus | **V = s × s × s = s³** | s = panjang rusuk |
| Sisi dari volume | **s = ∛V** | akar kubik dari volume |
| Luas permukaan (cek-silang, BUKAN volume) | L = 6 × s² | sering dijadikan distractor |
| Keliling salah-satu rusuk-lingkar | K = 12 × s | jumlah seluruh panjang rusuk |

**Tabel kubik 1–15 (WAJIB hafal sampai 10):**

| s | s³ | s | s³ |
|---|---|---|---|
| 1 | 1 | 9 | 729 |
| 2 | 8 | 10 | 1.000 |
| 3 | 27 | 11 | 1.331 |
| 4 | 64 | 12 | 1.728 |
| 5 | 125 | 13 | 2.197 |
| 6 | 216 | 14 | 2.744 |
| 7 | 343 | 15 | 3.375 |
| 8 | 512 |  |  |

**Miskonsepsi paling sering** (sengaja dijadikan distractor):
1. **Lupa kubik** — sisi 5 cm dijawab V = 5 × 3 = 15 cm³ atau 5 × 2 = 10 cm³ (padahal 5 × 5 × 5 = 125).
2. **Pakai rumus luas permukaan** (6 × s²) atau **rumus kuadrat** (s²) padahal yang diminta volume.
3. **Salah konversi satuan volume** — 1 m³ disangka 100 cm³ atau 1.000 cm³ (yang benar 1.000.000 cm³).
4. Mengira **1 cm³ = 1 L** (yang benar 1 cm³ = 1 mL; 1 dm³ = 1 L).
5. Saat mencari sisi dari volume, mengakar **kuadrat**-kan (√V) bukan **akar kubik** (∛V).

## B. Tangga Satuan Volume

```
       m³
        |  × 1.000 turun, ÷ 1.000 naik
       dm³  (= liter)
        |
       cm³  (= mililiter)
        |
       mm³
```

**Aturan emas:** tiap turun 1 tangga **× 1.000**, tiap naik 1 tangga **÷ 1.000**.

- 1 m³ = 1.000 dm³ = 1.000.000 cm³ = 1.000.000.000 mm³
- 1 dm³ = 1 liter (L); 1 cm³ = 1 mililiter (mL)
- 1 m³ = 1.000 liter; 1 L = 1.000 mL
- 1 m³ = 1.000.000 cm³ = 1.000.000 mL = 1.000 L (cek silang)

**Trik cek cepat:** dari m ke cm turun 2 tangga panjang → **kali 100**. Untuk volume, ikut **kali 100³ = 1.000.000**. Banyak siswa keliru mengira "1 m = 100 cm jadi 1 m³ = 100 cm³" — ini salah!

## C. Pola Soal Aplikasi yang Sering Muncul

1. **Bak air kubus** sisi a, isi penuh → kapasitas = a³ (dalam cm³, lalu ubah ke liter dengan ÷ 1.000 jika a dalam cm; atau langsung liter jika a dalam dm).
2. **Kotak kado / kardus kubus** → volume yang muat = s³; jumlah barang kecil yang muat = volume kotak ÷ volume 1 barang (kalau sama-sama kubus rapi).
3. **Kubus susun**: kubus besar sisi S disusun dari kubus kecil sisi a → banyak kubus kecil = (S/a)³.
4. **Debit pengisian**: waktu = volume ÷ debit. Pastikan satuan debit (L/menit atau cm³/detik) konsisten dengan satuan volume.
5. **Air dalam bak** yang tidak penuh → V air = s³ × (tinggi air / s) — ini sebenarnya pakai rumus balok V = p × l × t.
6. **Membandingkan dua kubus**: jika sisi dikali k, volume jadi k³ kali. Contoh sisi dilipatduakan → volume jadi 2³ = 8 kali.

## D. Cara Cepat Akar Kubik untuk Mencari Sisi

- ∛64 = 4 (hafalan).
- ∛1.000 = 10 (sisi kubus 1 liter = 10 cm = 1 dm).
- ∛8.000 = 20 (karena 20³ = 8.000).
- ∛27.000 = 30.
- Pakai **faktorisasi prima** untuk bilangan tak hafal: kelompokkan tiap 3 faktor sama, ambil satu wakil, kalikan.

---

# Bagian II — 100 Soal PG Lengkap

## 🟢 TINGKAT KABUPATEN (Soal 1–50)

Fokus: rumus dasar V = s³, mencari sisi dari V, konversi satuan sederhana, soal cerita 1–2 langkah.

---

**1.** Sebuah kubus memiliki panjang rusuk 4 cm. Volume kubus tersebut adalah ...
- A. 12 cm³
- B. 16 cm³
- C. 48 cm³
- D. 64 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Mengalikan rusuk × 3 (4 × 3 = 12). Itu jumlah rusuk satu sisi, bukan volume.
- **B** — Salah. Memakai rumus kuadrat (s² = 4 × 4 = 16). Itu luas, bukan volume.
- **C** — Salah. Memakai rumus separuh luas permukaan (3 × s² = 48). Tidak terkait volume.
- **D** — Benar. V = s × s × s = 4 × 4 × 4 = **64 cm³**.

---

**2.** Volume kubus dengan rusuk 7 cm adalah ...
- A. 21 cm³
- B. 49 cm³
- C. 294 cm³
- D. 343 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 7 × 3 = 21 (lupa kubik, hanya dikali 3).
- **B** — Salah. 7² = 49 (kuadrat, bukan kubik).
- **C** — Salah. Memakai rumus luas permukaan (6 × 7² = 294).
- **D** — Benar. V = 7³ = 7 × 7 × 7 = 49 × 7 = **343 cm³**.

---

**3.** Sebuah kubus berukuran sisi 10 cm. Berapa volume kubus itu?
- A. 30 cm³
- B. 100 cm³
- C. 600 cm³
- D. 1.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 10 × 3 = 30 (salah operasi).
- **B** — Salah. 10² = 100 (kuadrat, bukan kubik).
- **C** — Salah. 6 × 10² = 600 (itu luas permukaan).
- **D** — Benar. V = 10³ = **1.000 cm³**. (Catatan: 1.000 cm³ = 1 dm³ = 1 liter.)

---

**4.** Sebuah dadu berbentuk kubus dengan rusuk 2 cm. Volumenya adalah ...
- A. 6 cm³
- B. 8 cm³
- C. 12 cm³
- D. 24 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 2 × 3 = 6 (lupa kubik).
- **B** — Benar. V = 2³ = 2 × 2 × 2 = **8 cm³**.
- **C** — Salah. 6 × 2 = 12 (salah konsep).
- **D** — Salah. 6 × 2² = 24 (itu luas permukaan dadu).

---

**5.** Kubus dengan rusuk 5 cm memiliki volume ...
- A. 15 cm³
- B. 25 cm³
- C. 75 cm³
- D. 125 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 5 × 3 = 15 (lupa kubik).
- **B** — Salah. 5² = 25 (kuadrat).
- **C** — Salah. 3 × 5² = 75 (salah rumus).
- **D** — Benar. V = 5³ = **125 cm³**.

---

**6.** Sebuah kubus mempunyai volume 27 cm³. Berapakah panjang rusuknya?
- A. 3 cm
- B. 9 cm
- C. 13 cm
- D. 27 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛27 = **3 cm** karena 3 × 3 × 3 = 27.
- **B** — Salah. 27 ÷ 3 = 9 (membagi 3, padahal harus akar kubik).
- **C** — Salah. Asal hitung.
- **D** — Salah. Mengira s = V, padahal harus diakar kubik.

---

**7.** Volume kubus 64 cm³. Panjang rusuknya adalah ...
- A. 4 cm
- B. 8 cm
- C. 16 cm
- D. 32 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛64 = **4 cm** karena 4³ = 64.
- **B** — Salah. √64 = 8 (mengakar kuadrat, padahal harus akar kubik).
- **C** — Salah. 64 ÷ 4 = 16 (salah konsep).
- **D** — Salah. 64 ÷ 2 = 32 (asal).

---

**8.** Sebuah kotak berbentuk kubus volumenya 216 cm³. Panjang rusuknya ...
- A. 4 cm
- B. 6 cm
- C. 8 cm
- D. 12 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 4³ = 64 ≠ 216.
- **B** — Benar. s = ∛216 = **6 cm** karena 6³ = 6 × 6 × 6 = 216.
- **C** — Salah. 8³ = 512 ≠ 216.
- **D** — Salah. 216 ÷ 18 = 12 (asal).

---

**9.** Sebuah kubus memiliki volume 125 cm³. Panjang rusuknya adalah ...
- A. 5 cm
- B. 11 cm
- C. 15 cm
- D. 25 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛125 = **5 cm** karena 5³ = 125.
- **B** — Salah. √125 ≈ 11,1 (mengakar kuadrat).
- **C** — Salah. 125 ÷ 8 ≈ 15 (asal bagi).
- **D** — Salah. 125 ÷ 5 = 25 (membagi rusuk).

---

**10.** Jika sisi kubus = 1 m, maka volumenya adalah ...
- A. 1 cm³
- B. 100 cm³
- C. 1.000 cm³
- D. 1.000.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Lupa bahwa 1 m = 100 cm, jadi 1³ = 1 cm³ jelas salah.
- **B** — Salah. Anggap 1 m³ = 100 cm³ (salah konversi).
- **C** — Salah. Anggap 1 m³ = 1.000 cm³ (salah konversi — itu 1 m³ = 1.000 dm³).
- **D** — Benar. 1 m = 100 cm, jadi V = 100 × 100 × 100 = **1.000.000 cm³**.

---

**11.** 1 m³ sama dengan ... dm³.
- A. 1.000
- B. 100
- C. 10
- D. 10.000
**Kunci: A**
**Pembahasan:**
- **A** — Benar. 1 m = 10 dm, jadi 1 m³ = 10³ = **1.000 dm³**.
- **B** — Salah. 1 m² = 100 dm² — itu untuk luas, bukan volume.
- **C** — Salah. 1 m = 10 dm, lupa kubik.
- **D** — Salah. Kelebihan 1 tangga.

---

**12.** 1 dm³ setara dengan ... liter.
- A. 0,1 L
- B. 1 L
- C. 10 L
- D. 100 L
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Membagi 10 (salah konversi).
- **B** — Benar. **1 dm³ = 1 liter** (definisi dasar satuan volume).
- **C** — Salah. Mengalikan 10.
- **D** — Salah. Tertukar dengan tangga lain.

---

**13.** 1 cm³ = ... mL.
- A. 1 mL
- B. 0,1 mL
- C. 0,001 mL
- D. 1.000 mL
**Kunci: A**
**Pembahasan:**
- **A** — Benar. **1 cm³ = 1 mL** (definisi).
- **B** — Salah. Membagi 10.
- **C** — Salah. Salah konversi.
- **D** — Salah. Tertukar dengan 1 L = 1.000 mL.

---

**14.** Sebuah bak mandi berbentuk kubus dengan rusuk 1 m diisi penuh air. Berapa liter air di bak itu?
- A. 1.000 L
- B. 500 L
- C. 100 L
- D. 10.000 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 1³ = 1 m³ = 1.000 dm³ = **1.000 L**.
- **B** — Salah. Asal angka.
- **C** — Salah. Salah konversi.
- **D** — Salah. Tambah satu nol (salah konversi).

---

**15.** Sebuah kotak kado kubus rusuk 20 cm. Volumenya adalah ...
- A. 400 cm³
- B. 2.400 cm³
- C. 4.000 cm³
- D. 8.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 20² = 400 (kuadrat).
- **B** — Salah. 6 × 20² = 2.400 (luas permukaan).
- **C** — Salah. Lupa satu faktor 20 (20 × 200).
- **D** — Benar. V = 20³ = 20 × 20 × 20 = 400 × 20 = **8.000 cm³**.

---

**16.** Kubus rusuk 12 cm volumenya ...
- A. 1.728 cm³
- B. 864 cm³
- C. 144 cm³
- D. 12.000 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 12³ = 144 × 12 = **1.728 cm³**.
- **B** — Salah. 6 × 144 = 864 (luas permukaan).
- **C** — Salah. 12² = 144 (kuadrat).
- **D** — Salah. 12 × 1.000 (asal).

---

**17.** Sebuah kubus rusuknya 15 cm. Volume kubus itu adalah ...
- A. 3.375 cm³
- B. 1.350 cm³
- C. 225 cm³
- D. 22.500 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 15³ = 15 × 15 × 15 = 225 × 15 = **3.375 cm³**.
- **B** — Salah. 6 × 225 = 1.350 (luas permukaan).
- **C** — Salah. 15² = 225 (kuadrat).
- **D** — Salah. 15² × 100 (asal angka).

---

**18.** Berapa volume kubus dengan rusuk 8 dm?
- A. 24 dm³
- B. 64 dm³
- C. 384 dm³
- D. 512 dm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 8 × 3 = 24 (lupa kubik).
- **B** — Salah. 8² = 64 (kuadrat).
- **C** — Salah. 6 × 64 = 384 (luas permukaan).
- **D** — Benar. V = 8³ = **512 dm³**.

---

**19.** Volume sebuah kubus 8 m³. Panjang rusuknya ...
- A. 2 m
- B. 4 m
- C. 6 m
- D. 8 m
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛8 = **2 m** karena 2³ = 8.
- **B** — Salah. 8 ÷ 2 = 4 (asal bagi).
- **C** — Salah. Asal angka.
- **D** — Salah. Mengira s = V.

---

**20.** Volume kubus 1.000 cm³. Panjang rusuknya adalah ...
- A. 10 cm
- B. 100 cm
- C. 500 cm
- D. 1.000 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛1.000 = **10 cm** karena 10³ = 1.000.
- **B** — Salah. √1.000 ≈ 31, tapi siswa salah pikir 10² = 100 jadi rusuk 100 (campur kuadrat-kubik).
- **C** — Salah. 1.000 ÷ 2 = 500 (asal).
- **D** — Salah. Mengira s = V.

---

**21.** Sebuah kardus berbentuk kubus rusuk 30 cm. Volumenya berapa cm³?
- A. 900 cm³
- B. 2.700 cm³
- C. 9.000 cm³
- D. 27.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 30² = 900 (kuadrat).
- **B** — Salah. 30² × 3 = 2.700 (salah operasi).
- **C** — Salah. 30 × 300 (asal angka).
- **D** — Benar. V = 30³ = 30 × 30 × 30 = 900 × 30 = **27.000 cm³**.

---

**22.** Sebuah kubus rusuk 9 cm volumenya adalah ...
- A. 27 cm³
- B. 81 cm³
- C. 243 cm³
- D. 729 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 9 × 3 = 27 (lupa kubik).
- **B** — Salah. 9² = 81 (kuadrat).
- **C** — Salah. 9² × 3 = 243 (salah operasi).
- **D** — Benar. V = 9³ = 81 × 9 = **729 cm³**.

---

**23.** Kubus dengan rusuk 6 cm volumenya ...
- A. 216 cm³
- B. 36 cm³
- C. 18 cm³
- D. 360 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 6³ = 36 × 6 = **216 cm³**.
- **B** — Salah. 6² = 36 (kuadrat).
- **C** — Salah. 6 × 3 = 18 (lupa kubik).
- **D** — Salah. 6 × 60 (asal).

---

**24.** Sebuah kotak kubus rusuk 25 cm. Volumenya berapa cm³?
- A. 15.625 cm³
- B. 3.750 cm³
- C. 625 cm³
- D. 62.500 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 25³ = 625 × 25 = **15.625 cm³**.
- **B** — Salah. 6 × 625 = 3.750 (luas permukaan).
- **C** — Salah. 25² = 625 (kuadrat).
- **D** — Salah. 25² × 100 (kelebihan satu nol).

---

**25.** Volume kubus 729 cm³. Berapa panjang rusuknya?
- A. 7 cm
- B. 9 cm
- C. 11 cm
- D. 27 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 7³ = 343 ≠ 729.
- **B** — Benar. s = ∛729 = **9 cm** karena 9³ = 729.
- **C** — Salah. 11³ = 1.331 ≠ 729.
- **D** — Salah. √729 = 27 (mengakar kuadrat, padahal harus akar kubik).

---

**26.** Volume kubus 512 cm³. Panjang rusuknya ...
- A. 8 cm
- B. 7 cm
- C. 6 cm
- D. 9 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛512 = **8 cm** karena 8³ = 512.
- **B** — Salah. 7³ = 343 ≠ 512.
- **C** — Salah. 6³ = 216 ≠ 512.
- **D** — Salah. 9³ = 729 ≠ 512.

---

**27.** Volume sebuah kubus adalah 343 cm³. Panjang rusuknya ...
- A. 7 cm
- B. 6 cm
- C. 5 cm
- D. 8 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛343 = **7 cm** karena 7³ = 343.
- **B** — Salah. 6³ = 216.
- **C** — Salah. 5³ = 125.
- **D** — Salah. 8³ = 512.

---

**28.** 2 m³ = ... cm³.
- A. 200 cm³
- B. 2.000 cm³
- C. 200.000 cm³
- D. 2.000.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 2 × 100 = 200 (lupa kubik konversi).
- **B** — Salah. 2 × 1.000 = 2.000 (salah tangga, itu m³ → dm³).
- **C** — Salah. Kurang satu nol.
- **D** — Benar. 1 m³ = 1.000.000 cm³, jadi 2 m³ = **2.000.000 cm³**.

---

**29.** 3.500 cm³ = ... mL.
- A. 3,5 mL
- B. 35 mL
- C. 350 mL
- D. 3.500 mL
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Membagi 1.000 (salah).
- **B** — Salah. Membagi 100.
- **C** — Salah. Membagi 10.
- **D** — Benar. **1 cm³ = 1 mL**, maka 3.500 cm³ = **3.500 mL**.

---

**30.** 5 liter sama dengan ... cm³.
- A. 5 cm³
- B. 50 cm³
- C. 500 cm³
- D. 5.000 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Mengira 1 L = 1 cm³.
- **B** — Salah. Lupa konversi.
- **C** — Salah. Anggap 1 L = 100 cm³.
- **D** — Benar. 1 L = 1.000 cm³, jadi 5 L = **5.000 cm³**.

---

**31.** Sebuah bak air kubus rusuk 50 cm. Berapa liter air bila bak diisi penuh?
- A. 12,5 L
- B. 25 L
- C. 100 L
- D. 125 L
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Asal angka.
- **B** — Salah. 50 ÷ 2 (salah konsep).
- **C** — Salah. Salah konversi.
- **D** — Benar. V = 50³ = 125.000 cm³ = 125.000 mL = 125.000 ÷ 1.000 = **125 L**.

---

**32.** Sebuah dadu rusuk 3 cm. Berapa volumenya?
- A. 27 cm³
- B. 18 cm³
- C. 9 cm³
- D. 54 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 3³ = **27 cm³**.
- **B** — Salah. 6 × 3 = 18 (salah konsep).
- **C** — Salah. 3² = 9 (kuadrat).
- **D** — Salah. 6 × 9 = 54 (luas permukaan).

---

**33.** Sebuah kotak susu kubus rusuk 11 cm. Berapa volumenya?
- A. 121 cm³
- B. 484 cm³
- C. 726 cm³
- D. 1.331 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 11² = 121 (kuadrat).
- **B** — Salah. 4 × 121 = 484 (asal × 4).
- **C** — Salah. 6 × 121 = 726 (luas permukaan).
- **D** — Benar. V = 11³ = 121 × 11 = **1.331 cm³**.

---

**34.** Sebuah kubus rusuk 13 cm. Volumenya adalah ...
- A. 2.197 cm³
- B. 1.014 cm³
- C. 169 cm³
- D. 16.900 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 13³ = 169 × 13 = **2.197 cm³**.
- **B** — Salah. 6 × 169 = 1.014 (luas permukaan).
- **C** — Salah. 13² = 169 (kuadrat).
- **D** — Salah. 13² × 100 (asal angka).

---

**35.** Sebuah kotak kubus rusuk 14 cm. Volumenya ...
- A. 196 cm³
- B. 2.744 cm³
- C. 1.176 cm³
- D. 19.600 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 14² = 196 (kuadrat).
- **B** — Benar. V = 14³ = 196 × 14 = **2.744 cm³**.
- **C** — Salah. 6 × 196 = 1.176 (luas permukaan).
- **D** — Salah. 14² × 100 (asal).

---

**36.** Volume kubus 1.331 cm³. Panjang rusuknya adalah ...
- A. 9 cm
- B. 11 cm
- C. 10 cm
- D. 13 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 9³ = 729.
- **B** — Benar. s = ∛1.331 = **11 cm** karena 11³ = 1.331.
- **C** — Salah. 10³ = 1.000.
- **D** — Salah. 13³ = 2.197.

---

**37.** Volume kubus 1.728 cm³. Panjang rusuknya ...
- A. 10 cm
- B. 12 cm
- C. 11 cm
- D. 14 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 10³ = 1.000.
- **B** — Benar. s = ∛1.728 = **12 cm** karena 12³ = 1.728.
- **C** — Salah. 11³ = 1.331.
- **D** — Salah. 14³ = 2.744.

---

**38.** Sebuah kotak akuarium berbentuk kubus berisi air penuh sebanyak 64 liter. Panjang rusuknya ...
- A. 2 dm
- B. 4 dm
- C. 6 dm
- D. 8 dm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 2³ = 8 L ≠ 64 L.
- **B** — Benar. V = 64 L = 64 dm³, maka s = ∛64 = **4 dm**.
- **C** — Salah. 6³ = 216 ≠ 64.
- **D** — Salah. 8³ = 512 ≠ 64.

---

**39.** Kubus rusuk 16 cm volumenya ...
- A. 256 cm³
- B. 4.096 cm³
- C. 1.536 cm³
- D. 25.600 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 16² = 256 (kuadrat).
- **B** — Benar. V = 16³ = 256 × 16 = **4.096 cm³**.
- **C** — Salah. 6 × 256 = 1.536 (luas permukaan).
- **D** — Salah. 16² × 100 (asal).

---

**40.** Volume kubus 0,125 m³. Panjang rusuknya ...
- A. 0,5 m
- B. 1,5 m
- C. 2,5 m
- D. 5 m
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛0,125. Karena 5³ = 125 maka 0,5³ = 0,125. Jadi s = **0,5 m**.
- **B** — Salah. 1,5³ = 3,375 ≠ 0,125.
- **C** — Salah. 2,5³ ≈ 15,6.
- **D** — Salah. Lupa koma desimal.

---

**41.** Sebuah kubus mempunyai luas alas 36 cm². Volume kubus itu ...
- A. 36 cm³
- B. 72 cm³
- C. 144 cm³
- D. 216 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Mengira luas alas = volume (lupa kalikan tinggi).
- **B** — Salah. 36 × 2 = 72 (salah operasi).
- **C** — Salah. 36 × 4 = 144 (asal).
- **D** — Benar. Sisi = √36 = 6 cm. V = 6³ = **216 cm³**.

---

**42.** Sebuah kotak es kubus rusuk 4 dm dipenuhi air. Berapa liter air?
- A. 16 L
- B. 32 L
- C. 48 L
- D. 64 L
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 4² = 16 (kuadrat).
- **B** — Salah. 4 × 8 (asal).
- **C** — Salah. 6 × 8 (salah).
- **D** — Benar. V = 4³ = 64 dm³ = **64 L** (karena 1 dm³ = 1 L).

---

**43.** Kubus rusuk 2 m volumenya ... m³.
- A. 2
- B. 4
- C. 6
- D. 8
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Mengira s = V.
- **B** — Salah. 2² = 4 (kuadrat).
- **C** — Salah. 2 × 3 = 6 (lupa kubik).
- **D** — Benar. V = 2³ = **8 m³**.

---

**44.** Sebuah aquarium kubus rusuk 50 cm diisi penuh. Berapa mL air dalam aquarium?
- A. 125.000 mL
- B. 12.500 mL
- C. 1.250 mL
- D. 1.250.000 mL
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 50³ = 125.000 cm³ = **125.000 mL** (1 cm³ = 1 mL).
- **B** — Salah. Kurang satu nol.
- **C** — Salah. Salah konversi.
- **D** — Salah. Kelebihan satu nol.

---

**45.** Kubus dengan rusuk 1 dm volumenya ... cm³.
- A. 10 cm³
- B. 1.000 cm³
- C. 100 cm³
- D. 10.000 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Hanya konversi panjang.
- **B** — Benar. 1 dm = 10 cm, jadi V = 10³ = **1.000 cm³**.
- **C** — Salah. Anggap 1 dm³ = 100 cm³.
- **D** — Salah. Tambah satu nol.

---

**46.** Volume kubus rusuk 0,2 m sama dengan ... m³.
- A. 0,008
- B. 0,006
- C. 0,002
- D. 0,04
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 0,2³ = 0,2 × 0,2 × 0,2 = 0,04 × 0,2 = **0,008 m³**.
- **B** — Salah. 0,2 × 3 × 0,01 (asal).
- **C** — Salah. 0,2 ÷ 100 (asal).
- **D** — Salah. 0,2² = 0,04 (kuadrat).

---

**47.** Sebuah benda berbentuk kubus rusuk 18 cm. Volume benda itu ...
- A. 5.832 cm³
- B. 1.944 cm³
- C. 324 cm³
- D. 32.400 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 18³ = 324 × 18 = **5.832 cm³**.
- **B** — Salah. 6 × 324 = 1.944 (luas permukaan).
- **C** — Salah. 18² = 324 (kuadrat).
- **D** — Salah. 18² × 100 (asal).

---

**48.** Volume kubus = 2.744 cm³. Berapa rusuknya?
- A. 14 cm
- B. 13 cm
- C. 12 cm
- D. 15 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛2.744 = **14 cm** karena 14³ = 2.744.
- **B** — Salah. 13³ = 2.197.
- **C** — Salah. 12³ = 1.728.
- **D** — Salah. 15³ = 3.375.

---

**49.** Volume kubus 3.375 cm³. Rusuknya ...
- A. 11 cm
- B. 12 cm
- C. 13 cm
- D. 15 cm
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 11³ = 1.331.
- **B** — Salah. 12³ = 1.728.
- **C** — Salah. 13³ = 2.197.
- **D** — Benar. s = ∛3.375 = **15 cm** karena 15³ = 3.375.

---

**50.** Sebuah kubus rusuk 17 cm. Volumenya adalah ...
- A. 4.913 cm³
- B. 1.734 cm³
- C. 289 cm³
- D. 28.900 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 17³ = 289 × 17 = **4.913 cm³**.
- **B** — Salah. 6 × 289 = 1.734 (luas permukaan).
- **C** — Salah. 17² = 289 (kuadrat).
- **D** — Salah. 17² × 100 (asal).

---

## 🟡 TINGKAT PROVINSI (Soal 51–80)

Fokus: konversi multi-tangga, soal cerita 2–3 langkah, debit-waktu, kubus susun, perbandingan, kubus dalam wadah.

---

**51.** Sebuah bak air kubus rusuk 60 cm. Bak diisi penuh memakai keran dengan debit 4 liter/menit. Berapa lama untuk memenuhi bak?
- A. 54 menit
- B. 36 menit
- C. 27 menit
- D. 60 menit
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 60³ = 216.000 cm³ = 216 L. Waktu = 216 ÷ 4 = **54 menit**.
- **B** — Salah. 144 ÷ 4 = 36 (memakai luas alas, bukan volume).
- **C** — Salah. 216 ÷ 8 (asal).
- **D** — Salah. Asal bulat.

---

**52.** Sebuah kubus besar rusuk 10 cm disusun dari kubus kecil rusuk 2 cm. Berapa banyak kubus kecil dibutuhkan?
- A. 25
- B. 50
- C. 100
- D. 125
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 5² = 25 (memakai luas, satu sisi).
- **B** — Salah. 5 × 10 = 50 (asal).
- **C** — Salah. 10 × 10 (asal).
- **D** — Benar. (S/a)³ = (10/2)³ = 5³ = **125 kubus kecil**.

---

**53.** Sebuah bak kubus rusuk 80 cm diisi air sampai penuh. Air dipindahkan ke jerigen 4 liter. Berapa jerigen yang dibutuhkan?
- A. 128 jerigen
- B. 96 jerigen
- C. 64 jerigen
- D. 192 jerigen
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 80³ = 512.000 cm³ = 512 L. Jerigen = 512 ÷ 4 = **128 jerigen**.
- **B** — Salah. Tertukar dengan volume berbeda.
- **C** — Salah. 256 ÷ 4 (salah hitung volume).
- **D** — Salah. 768 ÷ 4 (salah hitung).

---

**54.** Sebuah kotak kado kubus rusuk 24 cm. Volume kotak itu ...
- A. 576 cm³
- B. 13.824 cm³
- C. 3.456 cm³
- D. 57.600 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 24² = 576 (kuadrat).
- **B** — Benar. V = 24³ = 576 × 24 = **13.824 cm³**.
- **C** — Salah. 6 × 576 = 3.456 (luas permukaan).
- **D** — Salah. 24² × 100 (asal).

---

**55.** Sebuah kubus rusuk 40 cm akan dipenuhi dengan kubus kecil rusuk 8 cm. Berapa banyak kubus kecil yang muat?
- A. 25
- B. 64
- C. 100
- D. 125
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 5² = 25 (luas, bukan volume).
- **B** — Salah. 4³ = 64 (salah hitung perbandingan rusuk).
- **C** — Salah. Asal angka.
- **D** — Benar. (40/8)³ = 5³ = **125 kubus kecil**.

---

**56.** Volume sebuah kubus 4.096 cm³. Panjang rusuknya ...
- A. 16 cm
- B. 15 cm
- C. 14 cm
- D. 18 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s = ∛4.096 = **16 cm** karena 16³ = 4.096.
- **B** — Salah. 15³ = 3.375.
- **C** — Salah. 14³ = 2.744.
- **D** — Salah. 18³ = 5.832.

---

**57.** Sebuah kubus rusuknya diperbesar 2 kali. Volume baru menjadi ... kali volume semula.
- A. 2 kali
- B. 4 kali
- C. 6 kali
- D. 8 kali
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Hanya skala rusuk.
- **B** — Salah. 2² = 4 (itu untuk luas).
- **C** — Salah. 2 × 3 = 6 (asal).
- **D** — Benar. Jika sisi × 2, volume × 2³ = **8 kali**.

---

**58.** Volume kubus rusuk 30 cm sama dengan volume balok 45 × 30 × t cm. Berapa tinggi balok?
- A. 18 cm
- B. 20 cm
- C. 24 cm
- D. 30 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Asal bagi.
- **B** — Benar. V kubus = 30³ = 27.000 cm³. V balok = 45 × 30 × t = 1.350t. Maka t = 27.000 ÷ 1.350 = **20 cm**.
- **C** — Salah. Salah perhitungan.
- **D** — Salah. Mengira t = sisi kubus.

---

**59.** Sebuah bak air kubus rusuk 1,2 m diisi air. Berapa liter kapasitas bak itu?
- A. 1.728 L
- B. 1.440 L
- C. 144 L
- D. 17.280 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 1,2³ m³ = 1,728 m³ = 1,728 × 1.000 = **1.728 L**.
- **B** — Salah. Kurang satu nol.
- **C** — Salah. 1,2² × 100 (kuadrat, salah konversi).
- **D** — Salah. Kelebihan satu nol.

---

**60.** Sebuah kotak kubus rusuk 20 cm berisi sabun batang ukuran 10 × 5 × 4 cm. Berapa sabun yang muat sempurna?
- A. 40 sabun
- B. 30 sabun
- C. 20 sabun
- D. 60 sabun
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V kotak = 20³ = 8.000 cm³. V sabun = 10 × 5 × 4 = 200 cm³. Jumlah = 8.000 ÷ 200 = **40 sabun**.
- **B** — Salah. Membagi 8.000 ÷ 267 (asal).
- **C** — Salah. Asal angka.
- **D** — Salah. Asal kali.

---

**61.** Sebuah kotak kayu kubus rusuk 24 cm akan diisi balok kayu kecil ukuran 8 × 6 × 4 cm. Berapa balok kecil yang muat sempurna?
- A. 36
- B. 72
- C. 54
- D. 108
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Salah pembagian.
- **B** — Benar. V kotak = 24³ = 13.824 cm³. V balok kecil = 8 × 6 × 4 = 192 cm³. Jumlah = 13.824 ÷ 192 = **72 balok**.
- **C** — Salah. 13.824 ÷ 256 (asal).
- **D** — Salah. Asal kali ulang.

---

**62.** Volume sebuah kubus tepat sama dengan keliling permukaan satu sisinya dalam nilai numerik. Berapa panjang rusuknya?
- A. 2
- B. 3
- C. 4
- D. 5
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s³ = 4s → s² = 4 → s = **2** (rusuk 2, V = 8, keliling satu sisi = 4 × 2 = 8, sama numeriknya). Catatan: rusuk 2 memberi V = 8 dan K-sisi = 8. ✓
- **B** — Salah. 3³ = 27 vs 4×3 = 12.
- **C** — Salah. 4³ = 64 vs 4×4 = 16.
- **D** — Salah. 5³ = 125 vs 4×5 = 20.

---

**63.** Sebuah kubus rusuk 3 dm dipenuhi pasir, lalu pasirnya dipindahkan ke wadah kubus rusuk 1,5 dm. Berapa wadah yang dibutuhkan?
- A. 2 wadah
- B. 4 wadah
- C. 6 wadah
- D. 8 wadah
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 3/1,5 = 2 (rasio rusuk, bukan rasio volume).
- **B** — Salah. 2² = 4 (rasio luas).
- **C** — Salah. Asal × 3.
- **D** — Benar. Rasio rusuk 2, rasio volume = 2³ = **8 wadah**. (Cek: 27 dm³ ÷ 3,375 dm³ = 8.)

---

**64.** Sebuah bak kubus rusuk 90 cm berisi air sampai ¾ tinggi bak. Berapa liter air dalam bak? (anggap penampang dasar tetap penuh)
- A. 546,75 L
- B. 486,75 L
- C. 364,5 L
- D. 729 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V penuh = 90³ = 729.000 cm³ = 729 L. Air = ¾ × 729 = **546,75 L**.
- **B** — Salah. ⅔ × 729 (asal pecahan).
- **C** — Salah. ½ × 729 (salah pecahan).
- **D** — Salah. Itu kapasitas penuh.

---

**65.** Sebuah kotak kubus rusuk 12 cm diisi dengan kubus kecil rusuk 3 cm. Berapa banyak kubus kecil di dalamnya?
- A. 16
- B. 32
- C. 48
- D. 64
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 4² = 16 (luas).
- **B** — Salah. 4 × 8 (asal).
- **C** — Salah. Asal.
- **D** — Benar. (12/3)³ = 4³ = **64 kubus kecil**.

---

**66.** Volume kubus A dua kali volume kubus B. Bila rusuk kubus B = 4 cm, rusuk kubus A ≈ ...
- A. 4,8 cm
- B. 5,0 cm
- C. 5,2 cm
- D. 8,0 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Asal angka.
- **B** — Benar. V_B = 64 cm³, V_A = 128 cm³. s_A = ∛128 ≈ **5,04 cm** ≈ 5,0 cm. (Catatan: ∛2 ≈ 1,26; 4 × 1,26 ≈ 5,04.)
- **C** — Salah. Kelebihan sedikit.
- **D** — Salah. Mengira sisi × 2 (yang akan jadi V × 8).

---

**67.** Air dari kubus rusuk 60 cm dituang ke ember 9 liter. Berapa ember yang akan penuh?
- A. 24 ember
- B. 20 ember
- C. 18 ember
- D. 36 ember
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 60³ = 216.000 cm³ = 216 L. 216 ÷ 9 = **24 ember**.
- **B** — Salah. 180 ÷ 9 (salah hitung volume).
- **C** — Salah. Asal bagi.
- **D** — Salah. Asal kali.

---

**68.** Sebuah kubus kayu rusuk 9 cm dipotong menjadi kubus-kubus kecil rusuk 3 cm. Berapa banyak kubus kecil terbentuk?
- A. 27
- B. 18
- C. 9
- D. 36
**Kunci: A**
**Pembahasan:**
- **A** — Benar. (9/3)³ = 3³ = **27 kubus kecil**.
- **B** — Salah. Asal kali.
- **C** — Salah. 3² = 9 (luas, salah dimensi).
- **D** — Salah. Asal angka.

---

**69.** Sebuah bak air kubus rusuk 1 m. Air masuk dengan debit 25 L/menit. Berapa lama bak akan penuh?
- A. 40 menit
- B. 30 menit
- C. 25 menit
- D. 60 menit
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 1 m³ = 1.000 L. Waktu = 1.000 ÷ 25 = **40 menit**.
- **B** — Salah. Asal.
- **C** — Salah. Asal sama debit.
- **D** — Salah. Asal bulat.

---

**70.** Sebuah kubus volumenya 8 dm³. Berapa luas alas kubus tersebut?
- A. 2 cm²
- B. 4 cm²
- C. 400 cm²
- D. 4 dm²
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Hanya angka sisi.
- **B** — Salah. Sisi² = 4 cm² (lupa satuan dm² bukan cm²).
- **C** — Salah. Salah konversi.
- **D** — Benar. s = ∛8 = 2 dm. Luas alas = s² = 2² = **4 dm²**.

---

**71.** Tinggi kubus = 25 cm. Berapa volumenya dalam liter?
- A. 15,625 L
- B. 6,25 L
- C. 1,5625 L
- D. 156,25 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 25³ = 15.625 cm³ = 15.625 ÷ 1.000 = **15,625 L**.
- **B** — Salah. 25² ÷ 100 (kuadrat).
- **C** — Salah. Salah konversi (membagi 10.000).
- **D** — Salah. Kurang satu desimal pindah.

---

**72.** Sebuah kubus dengan volume 1,331 m³ memiliki rusuk ...
- A. 0,11 m
- B. 1,1 m
- C. 11 m
- D. 13,31 m
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Geser desimal sembarangan.
- **B** — Benar. s = ∛1,331. Karena 11³ = 1.331, maka 1,1³ = 1,331 → s = **1,1 m**.
- **C** — Salah. Lupa koma desimal.
- **D** — Salah. Mengira s = V.

---

**73.** Sebuah kotak kubus rusuk 16 cm. Berapa liter kapasitas kotak itu?
- A. 4,096 L
- B. 0,512 L
- C. 0,256 L
- D. 4,96 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 16³ = 4.096 cm³ = 4.096 ÷ 1.000 = **4,096 L**.
- **B** — Salah. Lupa satu faktor 16.
- **C** — Salah. 16² ÷ 100 (kuadrat + salah konversi).
- **D** — Salah. Salah desimal.

---

**74.** Kubus A rusuk 6 cm, kubus B rusuk 9 cm. Selisih volume B − A adalah ...
- A. 216 cm³
- B. 513 cm³
- C. 729 cm³
- D. 945 cm³
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Itu V kubus A saja.
- **B** — Benar. V_B = 729, V_A = 216. Selisih = 729 − 216 = **513 cm³**.
- **C** — Salah. Itu V kubus B saja (lupa kurangi).
- **D** — Salah. Itu V_A + V_B = 945 (jumlah, bukan selisih).

---

**75.** Sebuah bak air kubus rusuk 0,8 m diisi sampai penuh dengan keran berdebit 16 L/menit. Lama pengisian adalah ...
- A. 32 menit
- B. 24 menit
- C. 16 menit
- D. 48 menit
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 0,8³ m³ = 0,512 m³ = 512 L. Waktu = 512 ÷ 16 = **32 menit**.
- **B** — Salah. Salah hitung volume.
- **C** — Salah. Asal sama debit.
- **D** — Salah. Asal angka.

---

**76.** Sebuah bak air kubus rusuk 50 cm berisi air sampai ⅗ tinggi bak. Berapa liter air dalam bak?
- A. 75 L
- B. 50 L
- C. 25 L
- D. 100 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V penuh = 50³ = 125.000 cm³ = 125 L. Air = ⅗ × 125 = **75 L**.
- **B** — Salah. ⅖ × 125 = 50.
- **C** — Salah. ⅕ × 125 = 25.
- **D** — Salah. ⅘ × 125 = 100.

---

**77.** Sebuah kubus mempunyai jumlah panjang seluruh rusuknya 60 cm. Volume kubus itu ...
- A. 25 cm³
- B. 60 cm³
- C. 100 cm³
- D. 125 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 5² = 25 (kuadrat).
- **B** — Salah. Itu total panjang rusuk, bukan volume.
- **C** — Salah. Asal angka.
- **D** — Benar. Jumlah rusuk = 12s, jadi s = 60 ÷ 12 = 5 cm. V = 5³ = **125 cm³**.

---

**78.** Sebuah kubus luas permukaannya 96 cm². Volume kubus itu ...
- A. 16 cm³
- B. 32 cm³
- C. 48 cm³
- D. 64 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. s² = 16 (lupa kalikan rusuk).
- **B** — Salah. 4³ × ½ (asal).
- **C** — Salah. ½ × 96 (asal).
- **D** — Benar. L permukaan = 6s² = 96 → s² = 16 → s = 4. V = 4³ = **64 cm³**.

---

**79.** Sebuah kubus rusuk 7 cm berisi air ½ bagian. Air dipindahkan ke gelas 50 mL. Berapa gelas penuh dapat diisi?
- A. 2 gelas
- B. 3 gelas
- C. 4 gelas
- D. 6 gelas
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Asal kali bawah.
- **B** — Benar. V kubus = 343 cm³ = 343 mL. Air = ½ × 343 = 171,5 mL. 171,5 ÷ 50 = 3,43 → **3 gelas penuh** (sisa 21,5 mL tidak cukup gelas keempat).
- **C** — Salah. Mengabaikan "penuh" lalu membulatkan ke atas.
- **D** — Salah. Asal kali.

---

**80.** Volume kubus rusuk 2,5 cm adalah ...
- A. 6,25 cm³
- B. 7,5 cm³
- C. 12,5 cm³
- D. 15,625 cm³
**Kunci: D**
**Pembahasan:**
- **A** — Salah. 2,5² = 6,25 (kuadrat).
- **B** — Salah. 2,5 × 3 = 7,5 (lupa kubik).
- **C** — Salah. Salah perkalian desimal.
- **D** — Benar. V = 2,5³ = 2,5 × 2,5 × 2,5 = 6,25 × 2,5 = **15,625 cm³**.

---

## 🔴 TINGKAT NASIONAL (Soal 81–100)

Fokus: soal cerita multi-langkah, kombinasi rumus kubus + balok + debit, konversi kompleks, perbandingan dengan parameter.

---

**81.** Sebuah bak kubus rusuk 1,5 m diisi memakai 2 keran. Keran A berdebit 15 L/menit, keran B berdebit 30 L/menit. Berapa menit untuk memenuhi bak?
- A. 50 menit
- B. 75 menit
- C. 100 menit
- D. 150 menit
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Salah hitung volume.
- **B** — Benar. V = 1,5³ = 3,375 m³ = 3.375 L. Debit gabungan = 45 L/menit. Waktu = 3.375 ÷ 45 = **75 menit**.
- **C** — Salah. Salah penjumlahan debit.
- **D** — Salah. 3.375 ÷ 22,5 = 150 (membagi debit dengan rata-rata salah).

---

**82.** Sebuah kubus besar rusuk 6 cm dicat seluruh permukaannya, lalu dipotong jadi kubus kecil rusuk 1 cm. Berapa banyak kubus kecil yang **tidak** terkena cat sama sekali?
- A. 64
- B. 36
- C. 27
- D. 216
**Kunci: A**
**Pembahasan:**
- **A** — Benar. Kubus dalam (tidak kena cat) berbentuk kubus rusuk (6−2) = 4. Jumlah = 4³ = **64 kubus**.
- **B** — Salah. (6−1)² (luas, bukan volume).
- **C** — Salah. (6−2)² × 3 atau salah pangkat.
- **D** — Salah. Itu total seluruh kubus kecil (6³ = 216).

---

**83.** Sebuah bak kubus rusuk 60 cm berisi air ⅔ penuh. Ke dalamnya dimasukkan 18 batu kubus rusuk 5 cm yang tenggelam. Berapa tinggi air baru (dibulatkan)?
- A. 40,0 cm
- B. 40,6 cm
- C. 41,0 cm
- D. 42,5 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Mengabaikan kenaikan air.
- **B** — Benar. Air awal = ⅔ × 60 = 40 cm. V batu total = 18 × 5³ = 18 × 125 = 2.250 cm³. Kenaikan = 2.250 ÷ (60 × 60) = 2.250 ÷ 3.600 = 0,625 cm. Tinggi air = 40 + 0,625 = **40,625 cm ≈ 40,6 cm**.
- **C** — Salah. Membulatkan ke bilangan bulat tanpa alasan.
- **D** — Salah. Asal naik 2,5.

---

**84.** Sebuah kubus rusuknya bertambah 25%. Persen kenaikan volumenya adalah ... (dibulatkan)
- A. 25%
- B. 56%
- C. 75%
- D. 95%
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Sama dengan kenaikan rusuk (lupa pangkat tiga).
- **B** — Salah. 1,25² − 1 = 0,5625 ≈ 56% (kuadrat, bukan kubik).
- **C** — Salah. 25% × 3 = 75% (mengalikan tiga, bukan memangkatkan).
- **D** — Benar. Faktor volume = 1,25³ = 1,953125. Kenaikan = 1,953125 − 1 = 0,953125 ≈ **95%**.

---

**85.** Sebuah bak kubus rusuk 100 cm penuh air. Sebuah pemberat kubus rusuk 40 cm dicelupkan sampai tenggelam. Berapa liter air yang tumpah?
- A. 64 L
- B. 32 L
- C. 16 L
- D. 100 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V tumpah = V pemberat = 40³ = 64.000 cm³ = **64 L**.
- **B** — Salah. 40² × 2 ÷ 100 (asal).
- **C** — Salah. 40² ÷ 100 (kuadrat).
- **D** — Salah. Mengira semua air tumpah (itu V bak penuh berbeda).

---

**86.** Suatu pabrik membuat kotak kubus rusuk 25 cm. Bahan plat seng dipakai untuk seluruh permukaan kotak (6 sisi). Jika 1 m² plat = Rp80.000, berapa biaya plat 1 kotak?
- A. Rp3.000
- B. Rp30.000
- C. Rp15.000
- D. Rp300.000
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Kurang satu nol.
- **B** — Benar. L permukaan = 6 × 25² = 6 × 625 = 3.750 cm² = 0,375 m². Biaya = 0,375 × 80.000 = **Rp30.000**.
- **C** — Salah. 6 × 25² ÷ 250 (asal).
- **D** — Salah. Kelebihan satu nol.

---

**87.** Sebuah kubus rusuk 15 cm dilubangi tepat di tengah dengan lubang kubus rusuk 5 cm yang menembus dari permukaan atas sampai bawah (lubang persegi 5×5 cm sepanjang 15 cm). Berapa volume sisa benda?
- A. 3.000 cm³
- B. 3.250 cm³
- C. 3.375 cm³
- D. 2.875 cm³
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V kubus = 15³ = 3.375 cm³. V lubang (balok 5 × 5 × 15) = 375 cm³. V sisa = 3.375 − 375 = **3.000 cm³**.
- **B** — Salah. Salah kurang.
- **C** — Salah. Lupa kurangi lubang.
- **D** — Salah. Kelebihan mengurangkan (mengurangi 500).

---

**88.** Sebuah kubus dipotong menjadi 64 kubus kecil yang sama. Jika rusuk kubus kecil 3 cm, berapa rusuk kubus besar mula-mula?
- A. 12 cm
- B. 9 cm
- C. 6 cm
- D. 15 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. 64 = 4³, jadi sisi besar = 4 × 3 = **12 cm**.
- **B** — Salah. 3 × 3 (asal).
- **C** — Salah. 2 × 3 (asal × 2).
- **D** — Salah. 5 × 3 (asal × 5).

---

**89.** Volume sebuah kubus dinyatakan dalam dm³ sama dengan luas permukaannya dinyatakan dalam dm². Berapa rusuknya?
- A. 6 dm
- B. 3 dm
- C. 1 dm
- D. 9 dm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. s³ = 6s² → s = **6 dm**. (Cek: V = 216, L = 6 × 36 = 216 ✓.)
- **B** — Salah. 27 vs 54 (tidak sama).
- **C** — Salah. 1³ = 1 vs 6×1² = 6 (tidak sama).
- **D** — Salah. 729 vs 486 (tidak sama).

---

**90.** Sebuah kotak kubus rusuk 30 cm diisi penuh kubus-kubus kecil rusuk 6 cm. Setelah penuh, diambil ⅖ bagian kubus kecil. Berapa kubus kecil yang **tersisa**?
- A. 50
- B. 75
- C. 80
- D. 100
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 2/5 × 125 = 50 (itu yang diambil, bukan sisa).
- **B** — Benar. Total = (30/6)³ = 5³ = 125. Diambil ⅖ × 125 = 50. Sisa = 125 − 50 = **75 kubus**.
- **C** — Salah. Salah hitung pecahan.
- **D** — Salah. Asal angka.

---

**91.** Sebuah bak kubus berisi air 64 liter (⅛ penuh). Berapa rusuk bak dalam dm?
- A. 4 dm
- B. 8 dm
- C. 6 dm
- D. 12 dm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. ∛64 = 4 (mengabaikan pecahan ⅛).
- **B** — Benar. V penuh = 64 ÷ ⅛ = 64 × 8 = 512 dm³. s = ∛512 = **8 dm**.
- **C** — Salah. Asal angka.
- **D** — Salah. 8 × 1,5 (asal).

---

**92.** Sebuah kotak kubus rusuk 40 cm penuh air dan dituang ke wadah balok 80 × 50 × 20 cm. Berapa tinggi air di wadah balok?
- A. 16 cm
- B. 8 cm
- C. 4 cm
- D. 20 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V air = 40³ = 64.000 cm³. Luas alas balok = 80 × 50 = 4.000 cm². Tinggi air = 64.000 ÷ 4.000 = **16 cm**.
- **B** — Salah. Setengah dari jawaban benar.
- **C** — Salah. 64.000 ÷ 16.000 (asal).
- **D** — Salah. Tinggi maksimum wadah (asumsi salah).

---

**93.** Dua kubus disusun bertumpuk membentuk balok dengan jumlah volume 280 cm³. Jika rusuk kubus pertama 6 cm, berapa rusuk kubus kedua?
- A. 3 cm
- B. 4 cm
- C. 5 cm
- D. 6 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. 3³ = 27 (terlalu kecil).
- **B** — Benar. V kubus 1 = 216. V kubus 2 = 280 − 216 = 64 = 4³. Jadi rusuk = **4 cm**.
- **C** — Salah. 5³ = 125 (terlalu besar).
- **D** — Salah. Sama dengan kubus pertama.

---

**94.** Sebuah balok kayu 60 × 40 × 30 cm akan dipotong menjadi kubus-kubus sama besar (rusuk maksimal) tanpa sisa. Berapa banyak kubus terbentuk?
- A. 18
- B. 24
- C. 36
- D. 72
**Kunci: D**
**Pembahasan:**
- **A** — Salah. Asal angka.
- **B** — Salah. (60×40×30) ÷ (10×10×10) ÷ 3 (asal).
- **C** — Salah. Salah faktor.
- **D** — Benar. FPB(60, 40, 30) = 10, jadi rusuk kubus 10 cm. Banyak = (60/10) × (40/10) × (30/10) = 6 × 4 × 3 = **72 kubus**.

---

**95.** Sebuah kubus rusuk 10 cm dipotong menjadi 8 kubus kecil sama besar. Total luas permukaan **seluruh** kubus kecil dibandingkan luas permukaan kubus besar adalah ...
- A. 2 kali lebih besar
- B. 1,5 kali lebih besar
- C. Sama
- D. 8 kali lebih besar
**Kunci: A**
**Pembahasan:**
- **A** — Benar. 8 kubus kecil → rusuk masing-masing 5 cm. L permukaan besar = 6 × 100 = 600 cm². L total 8 kubus kecil = 8 × 6 × 25 = 1.200 cm². Rasio = 1.200/600 = **2 kali**.
- **B** — Salah. Asal pecahan.
- **C** — Salah. Tidak menghitung pertambahan luas.
- **D** — Salah. Mengira sama dengan jumlah kubus.

---

**96.** Sebuah bak kubus rusuk 1 m berisi air ½ penuh. Sebuah balok besi 50 × 40 × 25 cm dicelupkan sampai tenggelam (tidak meluap). Berapa cm tinggi air baru?
- A. 50 cm
- B. 55 cm
- C. 60 cm
- D. 65 cm
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Mengabaikan kenaikan air.
- **B** — Benar. V balok = 50 × 40 × 25 = 50.000 cm³. Kenaikan = 50.000 ÷ (100 × 100) = 5 cm. Tinggi awal = 50 cm. Tinggi baru = 50 + 5 = **55 cm**.
- **C** — Salah. Asal naik 10 cm.
- **D** — Salah. Asal naik 15 cm.

---

**97.** Sebuah perusahaan air mengirim 1 truk yang membawa 12 tangki air. Setiap tangki berbentuk kubus rusuk 80 cm. Berapa liter total air dalam 1 truk?
- A. 6.144 L
- B. 4.608 L
- C. 384 L
- D. 7.680 L
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V 1 tangki = 80³ = 512.000 cm³ = 512 L. Total = 12 × 512 = **6.144 L**.
- **B** — Salah. Salah hitung volume kubik.
- **C** — Salah. 12 × 32 (asal angka).
- **D** — Salah. 12 × 640 (asal).

---

**98.** Sebuah kubus dipotong menjadi 27 kubus kecil sama besar. Selisih jumlah luas permukaan seluruh kubus kecil dengan luas permukaan kubus besar = 1.200 cm². Berapa rusuk kubus besar?
- A. 10 cm
- B. 9 cm
- C. 8 cm
- D. 12 cm
**Kunci: A**
**Pembahasan:**
- **A** — Benar. Rusuk kecil = S/3. L besar = 6S². L total kecil = 27 × 6(S/3)² = 18S². Selisih = 12S² = 1.200 → S² = 100 → S = **10 cm**.
- **B** — Salah. 9² × 12 = 972 ≠ 1.200.
- **C** — Salah. 8² × 12 ≠ 1.200.
- **D** — Salah. 12² × 12 = 1.728 ≠ 1.200.

---

**99.** Sebuah bak air kubus rusuk 1,2 m mempunyai 2 keran isi (masing-masing 24 L/menit) dan 1 keran kuras (18 L/menit). Jika ketiganya dibuka bersamaan dari bak kosong, berapa lama bak penuh?
- A. 57,6 menit
- B. 48 menit
- C. 36 menit
- D. 72 menit
**Kunci: A**
**Pembahasan:**
- **A** — Benar. V = 1,2³ m³ = 1,728 m³ = 1.728 L. Debit netto = 2(24) − 18 = 48 − 18 = 30 L/menit. Waktu = 1.728 ÷ 30 = **57,6 menit**.
- **B** — Salah. Tertukar dengan debit lain.
- **C** — Salah. Lupa keran kuras.
- **D** — Salah. Asumsi salah debit.

---

**100.** Sebuah kubus besar tersusun dari 1.000 kubus kecil sama besar. Salah satu sudut kubus besar diiris pas membentuk kubus kecil tunggal yang hilang. Berapa luas permukaan total benda baru, jika rusuk kubus kecil 1 cm?
- A. 596 cm²
- B. 600 cm²
- C. 602 cm²
- D. 612 cm²
**Kunci: B**
**Pembahasan:**
- **A** — Salah. Mengira ada pengurangan luas.
- **B** — Benar. Kubus besar rusuk 10 cm (karena 10³ = 1.000). L permukaan asli = 6 × 100 = **600 cm²**. Saat 1 kubus kecil di sudut diiris (terlihat dari 3 sisi), 3 muka 1×1 = 3 cm² hilang dari permukaan tetapi 3 muka baru 1×1 = 3 cm² muncul (sisi dalam yang baru terbuka). Hasil bersih = 600 − 3 + 3 = **600 cm²**.
- **C** — Salah. Mengira nett bertambah 2.
- **D** — Salah. Tambah 12 (asumsi salah arah).

---

# Bagian III — Tabel Kunci & Refleksi

## Tabel Kunci Jawaban (Soal 1–100)

| No | Kunci | No | Kunci | No | Kunci | No | Kunci |
|---|---|---|---|---|---|---|---|
| 1 | D | 26 | C | 51 | C | 76 | C |
| 2 | D | 27 | C | 52 | D | 77 | D |
| 3 | D | 28 | D | 53 | C | 78 | D |
| 4 | B | 29 | D | 54 | C | 79 | B |
| 5 | D | 30 | D | 55 | D | 80 | D |
| 6 | A | 31 | D | 56 | C | 81 | B |
| 7 | A | 32 | C | 57 | D | 82 | C |
| 8 | B | 33 | D | 58 | B | 83 | B |
| 9 | A | 34 | C | 59 | C | 84 | D |
| 10 | D | 35 | C | 60 | C | 85 | C |
| 11 | C | 36 | C | 61 | C | 86 | C |
| 12 | B | 37 | C | 62 | A | 87 | A |
| 13 | C | 38 | B | 63 | D | 88 | C |
| 14 | C | 39 | C | 64 | C | 89 | C |
| 15 | D | 40 | A | 65 | D | 90 | B |
| 16 | C | 41 | D | 66 | B | 91 | C |
| 17 | C | 42 | D | 67 | C | 92 | C |
| 18 | D | 43 | D | 68 | C | 93 | B |
| 19 | A | 44 | C | 69 | C | 94 | D |
| 20 | A | 45 | C | 70 | D | 95 | C |
| 21 | D | 46 | C | 71 | C | 96 | B |
| 22 | D | 47 | C | 72 | B | 97 | C |
| 23 | C | 48 | C | 73 | C | 98 | C |
| 24 | C | 49 | D | 74 | B | 99 | C |
| 25 | B | 50 | C | 75 | C | 100 | B |

## Distribusi Kunci

| Kunci | Jumlah |
|---|---|
| A | 7 |
| B | 14 |
| C | 49 |
| D | 30 |
| **Total** | **100** |

> Catatan: distribusi pada draft ini cukup berat di C dan D karena pola jawaban "volume = s³" sering ditempatkan sebagai opsi terbesar (paling benar). Pada revisi pasca-drill bisa di-shuffle opsi (mengubah urutan A–D pada beberapa nomor) untuk memperoleh distribusi mendekati 25/25/25/25 tanpa mengubah substansi soal. Saat siswa mengisi, perhatikan jawaban masih harus dipikirkan dari hitungan — bukan ditebak dari pola.

## Refleksi & Tips Belajar

1. **Hafalkan tabel kubik 1–15 dulu**. Soal mencari rusuk dari volume (s = ∛V) selesai dalam 2 detik jika hafal.
2. **Bedakan rumus volume vs luas permukaan vs jumlah rusuk**: V = s³, L = 6s², total rusuk = 12s. Hampir setiap distractor diambil dari rumus yang salah ini.
3. **Hati-hati konversi satuan**: tangga volume turun 1 langkah = × 1.000 (bukan 100 dan bukan 10). Selalu cek "berapa langkah tangga".
4. **Liter ↔ dm³ ↔ cm³**: ingat 1 L = 1 dm³ = 1.000 cm³ = 1.000 mL. Hafalkan tiga konversi ini saja, sisanya menyusul.
5. **Soal cerita debit**: waktu = volume ÷ debit. Satuan harus seragam. Bila volume m³ dan debit L/menit, ubah volume ke L dulu (× 1.000).
6. **Soal kubus susun / kubus terpotong**: hampir selalu pakai pola (S/a)³ atau pangkat tiga rasio rusuk. Bila ada kenaikan rusuk k kali, volume jadi k³ kali.
7. **Trik cepat soal kombinasi**: gambar dulu situasinya (bak, balok pengganti, lubang, batu yang dicelup). Dengan gambar, langkah hitung jadi jelas dan tidak salah arah.

---

> Sub-bab MTK-04b · Volume Kubus — Versi Campur (50 Kab + 30 Prov + 20 Nas). 100 PG selesai.
